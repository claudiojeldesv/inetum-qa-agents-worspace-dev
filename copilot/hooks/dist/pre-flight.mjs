...
// src/compliance-preflight.ts
var compliance_preflight_exports = {};
__export(compliance_preflight_exports, {
  checkUrl: () => checkUrl,
...
function loadConfig(configPath) {
  const path = configPath ?? resolve2(process.cwd(), "config/allowed-targets.yaml");
  if (!existsSync2(path)) {
...
function runPreflight(url, configPath) {
  let config;
  try {
    config = loadConfig(configPath);
  } catch (err) {
    const reason = err instanceof Error ? err.message : String(err);
    const res = { verdict: "block", rule: "C3", reason, url };
    appendAuditEntry({
      source: "pre-flight",
      action: "block",
      target: url,
...
var import_yaml, FORBIDDEN_HARDCODED, SOFT_PREFIX_HINTS;
var init_compliance_preflight = __esm({
  "src/compliance-preflight.ts"() {
    "use strict";
...
// copilot/hooks/pre-flight.ts
function denyExit(reason) {
...
function extractUrl(payload) {
  ...
  const candidate = input.url ?? input.target ?? input.base_url;
...
async function main() {
  ...
  let runPreflight2;
  try {
    ({ runPreflight: runPreflight2 } = await Promise.resolve().then(() => (init_compliance_preflight(), compliance_preflight_exports)));
  } catch (err) {
    process.stderr.write(
      `[pre-flight] No puedo cargar el validador de compliance (\xBFfalta \`npm install\`?). Bloqueo por seguridad. (${err})
`
    );
    return denyExit(
      "[pre-flight] Validador de compliance no disponible (\xBFfalta npm install?). Fail-closed: tool denegada."
    );
  }
  const result = runPreflight2(url);
