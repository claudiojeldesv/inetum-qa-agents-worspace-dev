...
- `--update-spec=<path>` — **optional, incremental mode only**. An EXISTING `.spec.ts` (generated by
  a previous run) whose criterion changed. When present you EDIT that file instead of writing a new
  one (see "Update mode" below). `--output` is not passed in this mode.
- `--impact-entry=<json>` — **optional, incremental mode only**. The `modified` entry from
  `impact-report.json` for this spec: `{ rf, changed_fields, spec_files }`. Tells you exactly which
  criterion fields changed — the scope of your edit.
...
## Update mode (incremental, when `--update-spec` is present)

The spec already exists and was approved in a previous run; its criterion changed. You are a
surgeon, not a rewriter:

1. Read the existing spec whole. Read the criterion's NEW `given`/`when`/`then` in `criteria.json`
   and the `changed_fields` from `--impact-entry`.
2. **Edit only what the change demands.** `then` changed → adjust the closing asserts; `when`
   changed → adjust the action steps; `given` changed → adjust the setup/fixtures; `flow`/`title`
   changed → adjust naming. Locators, POM imports, tags, a11y scan and evidence instrumentation
   that remain valid are NOT touched.
3. **Preserve identity**: same file, same filename, same `@tc-id` (the stable registry ID survives
   the requirement change — that is its purpose). Update the `@criterion` JSDoc to the new
   `source_ref` and bump `@writer-iterations`.
4. Invoke the Reviewer on the edited file exactly like a new spec (same ping-pong, N≤2). An edited
   spec is not "already reviewed" — the previous approval covered the previous criterion.
5. Audit log: `{ source: 'subagent', action: 'edit_file', target: <update-spec>, rule: 'writer-update-mode', metadata: { rf, changed_fields } }`.

Never regenerate the file from scratch in update mode (you would lose healer fixes and reviewed
locators the previous run paid for). If the change is so deep that surgery is impossible (e.g. the
flow itself was redefined), stop and report it — the command will route it as remove+add, a QA
decision, not yours.
