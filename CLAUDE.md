...
- **v0.2 Fase B — EN CURSO** (detalle en [`docs/findings/wild-sites-report.md`](docs/findings/wild-sites-report.md)): producción contra sitios reales con catalogación incidental, brief happy-path acotado (plumbing `--flows/--entry/--ignore`, ya commiteado). **Hechos: sitio 1 expandtesting, sitio 2 Toolshop**. Dato estrella: el brief **escala a sitios grandes** (Toolshop catálogo grande → 10 tool-uses vs 62 del ciego). Hallazgos clave: a11y todo-o-nada aborta (1/2), W1 en todo sitio real (2/2), locator id-dinámico frágil (locator-hardener), `baseURL` hardcoded saucedemo (FIXED esta sesión: parametrizable por `QA_BASE_URL`), style-contract por sitio. **Siguiente: sitio 3 `parabank.parasoft.com`** (auth + estado, categoría auth-handler aún no observada). Luego: ajustes Quality layer + S3 → telemetría + budget cap.
