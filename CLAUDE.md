...
| Módulo | Entrada | Estado v0.2 |
|---|---|---|
| **S1 Code-driven** | Repo frontend (React/Vue/HTML) | Stub documentado (v0.3) |
| **S2 Req-driven** | Gherkin `.feature` + URL | **Funcional** (Fase E; OpenAPI diferido a v0.4) |
| **S3 Spec-refiner** | FD markdown libre + URL (Forma B) | **Funcional** (Fase D) |
| **S4 Autonomous (MCP)** | Solo URL | **Funcional** (desde v0.1) |
...
Snapshot vigente. El histórico de fases cerradas (A-F, releases, reorganización) vive en [`docs/findings/estado-historico.md`](docs/findings/estado-historico.md) — leerlo solo cuando la tarea necesite la arqueología de una fase concreta. Cronología de releases en [`CHANGELOG.md`](CHANGELOG.md).
...
- **Red de seguridad**: healthcheck 18/18, unit 72/72, build y lint verdes. Ejecutarla tras cambios al núcleo.
- **Pendientes conocidos**: `ia4d-mode-router` sin `Write` (no persiste `mode-routing.json`; funciona por respuesta de texto); tuning Writer↔Reviewer N=3 y judge axes por contract; baseline-diff de a11y (v0.2.x). Contra prod de terceros, el anti-bot es el límite operativo, no la capacidad del agente (lección Fase F).
- **Principio del QA**: la sanación va al final como post-proceso (Healer), no acoplada a la generación.
...
| [`docs/findings/estado-historico.md`](docs/findings/estado-historico.md) | Arqueología de fases cerradas (A-F, releases, reorganización) — solo si la tarea toca una fase concreta |
