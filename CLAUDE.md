...
- **Siguiente — v0.2 Fase D / S3 (Spec-refiner), decidido como FORMA B**: FD + URL = **inyección de criterios sobre el motor S4 validado** (reusa discovery/POM/Writer/Reviewer/Judge + los 3 componentes de C). El FD genera el brief (`--flows/--entry`) y los criterios RF-NNN citables; el planner pasa de descubrir a mapear-contra-DOM. Desbloquea **trazabilidad RF-NNN** y **detección de drift FD↔implementación** (reporta el gap, no fabrica). Forma A (FD-only, sin target) descartada: rompe el run-verde/locators-reales. **Jira/tickets/OpenAPI (S2) diferidos al final.** El resto de Fase D (Writer↔Reviewer N=3, judge axes por contract) es tuning menor, no bloquea S3.
