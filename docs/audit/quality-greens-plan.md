...
| Q5 (shift-left pre-review) | 2026-07-22 | n/a (validación de proceso, no de generación) | n/a | ~$2 A/B (8 writers) | n/a | métricas propias ↓ | **ADOPTADO**. Paridad (Sonnet+SL vs F6-A): tratamiento inerte — 0 intercepciones, 0 escapes, verificación idéntica 2/4. Estrés (Haiku+SL vs F6-B): MF mecánicos→Reviewer **0** (F6-B:9), invocaciones Reviewer **6** (F6-B:10), escapes 11.c **0** (F6-B:1), rescate a11y **0** (F6-B:1). Criterio (paridad limpio Y estrés ≥2/3) cumplido: estrés 3/3. Ver Notas Q5 |

### Notas Q5 — shift-left del pre-review (2026-07-22, artefactos en `.work/audit-runs/q5-arm-{parity,stress}/`)

**Cambio (uno, de prompt):** sección "Pre-review determinístico (shift-left)" en `ia4d-writer.md` — tras escribir el spec y tras cada corrección, ANTES de invocar al Reviewer, el Writer corre `pre-review.ts <output> --style-contract=… --out-dir=<workDir>/pre-review`, corrige los must-fix hasta clean (máx 2 pasadas; si no llega, invoca al Reviewer igual — N≤2 intacto). Nota de riesgo explícita contra "corregir para pasar el regex" (el `// css-fallback:` sin atributo sancionado no es corrección). La red 11.c post-review NO se movió (defensa en profundidad). `build:template` propagado (el script ya viajaba; los agentes ia4d los sirve el plugin, no el workspace — build:template es no-op para el .md del Writer, la fuente de verdad es `.claude/agents/`). Red estructural: tsc, 263/263 unit, healthcheck 26/26.

**Método:** A/B congelado doble sobre el discovery `frozen-fase3` (el MISMO input que los controles F6, legacy sin anotaciones `verified`, sin `--owned-poms` — solo varía el tratamiento + el modelo del Writer). Los controles F6-A/F6-B están ya medidos; se corrieron solo los brazos con tratamiento (4 writers cada uno, escalonados, foreground, Reviewer inline N≤2). Salvedad honesta: el Writer actual lleva el endurecimiento Q1-Q4 (API axe, reglas de locator), que F6 no tenía — sesga a favor de la no-degradación, refuerza (no debilita) la conclusión de paridad.

**Brazo paridad (Sonnet + shift-left) vs F6-A (4/4 iter-0, 0 must-fix, 8 should-fix):** el tratamiento salió **inerte en el camino normal** — el pre-review devolvió `must_fix:0 clean:true` en los 4 specs (0 intercepciones), luego NO modificó ningún spec, así que el Reviewer vio exactamente lo mismo que sin shift-left. Verdicts: TC-001/003/004 approved iter-0; TC-002 fue a iter-1 por un MF-8 (POM: precio inline vía `page.getByText` en vez de método del `CartPage`) — un must-fix de **juicio del Reviewer, no mecánico**, y verificablemente ajeno al tratamiento (shift-left encontró 0 en ese spec). Verificación 2/4 verde con el MISMO par de rojos que F6-A (clase carrito/checkout contra DOM, ortogonal a Q5). Riqueza del feedback preservada (should-fixes de password/precio siguen surgiendo). **Sin degradación atribuible al tratamiento → paridad limpia.**

**Brazo estrés (Haiku + shift-left) vs F6-B (9 must-fix al Reviewer, 10 invocaciones, 1 escape a 11.c, 1 rescate a11y):** las 3 métricas discriminantes mejoran (3/3, no solo ≥2):
- **must-fix mecánicos que llegan al Reviewer: 0** (F6-B: 9). El shift-left interceptó en iter-0 y con corrección propia (no dodge): TC-003 un locator de motor `text=/Total:/` (MF-1) → `getByText` en POM; TC-004 un CSS bruto `.error-message-container` (MF-1) → `loginPage.error`/`errorButton` + `toHaveClass(/\berror\b/)` anclado. Ambos habrían ido al Reviewer en F6-B. Los 2 must-fix que SÍ llegaron al Reviewer (TC-002) son `locator-strategy` "verificado en ejecución real" — wildcard `[data-test*="inventory-item"]` en los POMs, fabricación cazada corriendo el DOM: la clase **semántica** que verify-locators/Q2 ataca (aquí desactivada para igualar a F6), NO la mecánica que el pre-review cubre. Confirmado: cero `.locator('css')` a nivel de spec en el brazo.
- **invocaciones del Reviewer: 6** (1+2+1+2; F6-B: 10).
- **escapes a 11.c: 0** (F6-B: 1); y **rescate a11y: 0** (F6-B: 1 — el MF-4 del shift-left garantiza el scan bien colocado).
TC-002 acabó `iteration_2_exhausted` + rojo (la Quality layer hizo su trabajo: cazó la fabricación, agotó N≤2, escaló) — mismo desenlace de clase que F6. Verificación 2/4 verde, mismo patrón que F6-B.

**Riesgo inspeccionado (corregir "para pasar el regex"):** los diffs de corrección del brazo estrés se revisaron a mano — TC-003/TC-004 son fixes de raíz (locator semántico en POM, regex anclado), no evasiones del check. El único caso `// css-fallback:` no apareció.

**Decisión: ADOPTADO** — paridad limpia (tratamiento inerte, cero cambio de comportamiento atribuible) Y estrés mejora 3/3. El valor es robustez del proceso (seguro asimétrico: coste ~0 en el camino común Sonnet, pago grande en días malos / sitios raros / si se reabre el Writer barato), no el $ del run actual. Guardrails intactos: Quality layer sin tocar (Writer/Reviewer Sonnet, N≤2, Judge off), coste del A/B ~$2 (controles ya pagados en F6).

**Nota menor flaggeada (fuera de scope Q5):** un subagent-Writer reportó que `appendAuditEntry` colgó/no dejó rastro al loguear `write_file` desde dentro del subagent (la auditoría quedó cubierta por los hooks automáticos). No afecta al shift-left (prompt-only); candidato a revisar aparte si se reproduce.
