...
Historial completo de fases, releases y hallazgos en [`docs/STATUS.md`](docs/STATUS.md) (las entradas nuevas se añaden ARRIBA allí; aquí solo las 3-4 más recientes, comprimidas). Los defectos D-NN están catalogados en [`docs/references/indice-defectos.md`](docs/references/indice-defectos.md) — 67 a fecha 2026-08-30, con guarda mecánica.

- **P6 · la etiqueta de oráculo con dientes (2026-08-30)** — cada criterio arrastra de dónde salió su oráculo (`fd`|`app`|`captura`): `walk-to-spec` lo estampa en el JSDoc del test leyendo el acta del sitio (`@oraculo s6 app — «Employee Information» firmado por claudio.jeldes…`) y `pre-review` lo LEE y agrega `oraculos: {fd, app, captura, specs_sin_etiqueta}` al resumen — el número que dice si la suite se está volviendo un espejo de la aplicación. Acta rota → sin etiqueta (afirmar «fd» sin mirar sería mentir). Validado contra OrangeHRM real con el acta de campo. Detalle en P6 de [`docs/tasks/plan-panel-y-acta.md`](docs/tasks/plan-panel-y-acta.md).
- **P5 completo · la propuesta de FD (2026-08-30)** — lo que el QA firma en el acta ya llega al documento: `npm run qa:propuesta` emite `<fd>.propuesta.md` determinista, derivado SOLO de decisiones firmadas, con el origen de cada cambio en su fila (actor · grado · fecha · hash) y sin tocar jamás el FD del cliente. Un literal repetido o ausente no se adivina: «no ubicable». El primer uso real produjo los 3 cambios del ciclo de campo — incluido «(110) Records Found» con su aviso mecánico de contador — y encontró D67 (el original anclado es por sitio, los guiones por caso; mitigado con `--original=`). Detalle en P5 de [`docs/tasks/plan-panel-y-acta.md`](docs/tasks/plan-panel-y-acta.md).
- **P7 · el banco de pruebas de paneles (2026-08-29)** — los modales ya se prueban **a máquina contra el sitio real**: `npm run qa:bench` conduce el walker de verdad con un escenario JSON, despachando los mismos gestos que pulsaría el QA por el canal `qa-assist-cmd`. Medido contra OrangeHRM: 30/30 en textos, 10/10 sellado, y el ciclo entero paneles → firmas → fusión → **6/8 sin asistencia**. Dos guardas sin parámetro posible: firma como `banco-de-pruebas` y acta dentro de su work-dir — máquina jamás firma como persona. Encontró D64 (el panel tapa el objetivo y la verificación muere en jerga) mientras se construía; D64 mordió después al QA dos runs seguidos y se cerró el mismo día (`panelDejaDeInterceptar`), junto con D65 (el motivo del bloqueo afirmaba «se verificó» sin mirar `verified`) — el par falsable fue el propio banco sin `mover_panel`, 10/10. **El protocolo que instaura: el banco itera; el QA estrena** — toda rebanada de panel se audita con el banco ANTES de gastar la primera vez de una persona, que se reserva para lo único que no se puede automatizar: si el panel se entiende. Detalle: [`docs/findings/banco-de-pruebas-paneles.md`](docs/findings/banco-de-pruebas-paneles.md); protocolo en P7 de [`docs/tasks/plan-panel-y-acta.md`](docs/tasks/plan-panel-y-acta.md).
