...
/**
 * Ruta al `cli.js` de Playwright. Se resuelve por el ALGORITMO DE MÓDULOS
 * (desde el paquete, subiendo a su directorio) y no por una ruta relativa
 * cosida: un workspace de campo puede hoistear `node_modules` distinto, y ahí
 * `../../node_modules/...` no existiría. `@playwright/test/cli.js` no es un
 * subpath exportado, así que se resuelve el paquete y se compone desde su raíz.
 * Fail-closed: si no aparece, se dice — jamás se cae a `npx` con shell, que es
 * justo lo que destroza los argumentos.
 */
function resolverCliJs(): string {
  const req = createRequire(import.meta.url);
  const entry = req.resolve('@playwright/test'); // <pkg>/index.js
  const cli = resolve(entry, '..', 'cli.js');
  if (!existsSync(cli)) {
    console.error(`[qa:browse] no encuentro el cli.js de Playwright junto a ${entry}. ¿Está instalado @playwright/test?`);
    process.exit(1);
  }
  return cli;
}
